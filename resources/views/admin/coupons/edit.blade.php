...
@section('content')
    ...
    <section class="content">
        <div class="card">
            <div class="card-body">
                <form method="post" action="{{ URL::to(route('coupons.update', ['coupon' => $model->id])) }}">
                    @method("PUT")
                    {{csrf_field()}}
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="discount_value" value="{{old('discount_value', $model->discount_value)}}" class="form-control" />
                    </div>

                    <div class="form-group">
                        <label>Discount type: *</label>
                        <select name="discount_type" class="form-control">
                            <option value="1" @if(old('discount_type', $model->discount_type) == 1) selected="selected" @endif>Percent</option>
                            <option value="2" @if(old('discount_type', $model->discount_type) == 2) selected="selected" @endif>Amount</option>
                        </select>
                    </div>
